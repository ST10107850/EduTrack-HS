...
const LoginForm: React.FC = () => {
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await handleLogin(values.email, values.password); // Log in the user
            resetForm(); // Reset the form fields
            navigate('/parent-dashboard'); // Navigate to parent dashboard on successful login
        } catch (error) {
            // Capture and set the error message
            setError(error instanceof Error ? error.message : "Login failed"); // Show the error if login fails
        }
    };
    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-white"> {/* Change background to white */}
            <form onSubmit={onSubmit} className="bg-white p-8 rounded shadow-md w-96">
                <h2 className="text-2xl font-bold text-center mb-4 text-primaryColor">Login</h2>
